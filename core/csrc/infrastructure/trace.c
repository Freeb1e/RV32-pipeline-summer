...
static char call_stack[32][128]; // 调用栈，记录函数名
...
void init_trace(const char *elf_file)
{
  ...
  ftrace_set(true);
  Log("Ftrace: %s - Loaded %d function symbols", ANSI_FMT("ON", ANSI_COLOR_GREEN), func_num);
  Log("Function trace format: PC: [call/ret] function@address (depth: N)");
  Log("Use tree-like indentation: | +- +- to show call hierarchy");
}
...
{
  if (!ftrace_enable || stack_depth <= 0)
    return;

  Log(ANSI_COLOR_CYAN "=== Current Call Stack (depth: %d) ===" ANSI_COLOR_RESET, stack_depth);
  for (int i = 0; i < stack_depth && i < 32; i++)
  {
    char indent[64] = "";
    for (int j = 0; j < i; j++)
    {
    ...
    strcat(indent, i == stack_depth - 1 ? "+- " : "+- ");

    Log("%s " ANSI_COLOR_YELLOW "%s" ANSI_COLOR_RESET " " ANSI_DIM "(level %d)" ANSI_COLOR_RESET,
        indent, call_stack[i], i);
  }
  Log(ANSI_COLOR_CYAN "=================================" ANSI_COLOR_RESET);
}
...
void ftrace_decode(int rd, int rs, vaddr_t pc, vaddr_t dnpc)
{
  ...
  { // func call
    ...
    if (stack_depth < 32)
    {
      strncpy(call_stack[stack_depth], func_name, 127);
      call_stack[stack_depth][127] = '\0';
    }

    // 生成缩进字符串，使用更清晰的层次结构
    char *p_indent = indent;
    for (int i = 0; i < num_of_func_calls; i++)
    {
      p_indent += snprintf(p_indent, sizeof(indent) - (p_indent - indent), "|  ");
    }
    p_indent += snprintf(p_indent, sizeof(indent) - (p_indent - indent), "+- ");

    snprintf(msg, sizeof(msg),
             ANSI_COLOR_CYAN FMT_WORD ANSI_COLOR_RESET ": %s " ANSI_COLOR_GREEN "call" ANSI_COLOR_RESET " " ANSI_BOLD ANSI_COLOR_YELLOW "%s" ANSI_COLOR_RESET ANSI_COLOR_MAGENTA "@" FMT_WORD ANSI_COLOR_RESET
                                                       " " ANSI_DIM "(depth: %d)" ANSI_COLOR_RESET,
             pc, indent, func_name, dnpc, num_of_func_calls + 1);
  ...
  { // func return
    ...
    // 检查函数名是否匹配
    bool name_match = (stack_depth >= 0 && stack_depth < 32 &&
                       strcmp(call_stack[stack_depth], func_name) == 0);

    snprintf(msg, sizeof(msg),
             ANSI_COLOR_CYAN FMT_WORD ANSI_COLOR_RESET ": %s " ANSI_COLOR_RED "ret " ANSI_COLOR_RESET " " ANSI_BOLD ANSI_COLOR_YELLOW "%s" ANSI_COLOR_RESET
                                                       "%s"
                                                       " " ANSI_DIM "(depth: %d)" ANSI_COLOR_RESET,
             pc, indent, func_name,
             name_match ? "" : ANSI_COLOR_RED " [MISMATCH!]" ANSI_COLOR_RESET,
             num_of_func_calls);
